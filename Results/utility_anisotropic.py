# Basic utility functions used by all simulation codes for Aniosotropic model..
...
def cnot(bit1, bit2, errors, errorRates): 
  cnot0(bit1, bit2, errors)
  anisotropic_cx(bit1, bit2, errors, errorRates.cnot)
  errors1(bit1, errors, errorRates.prep)
  errors1(bit2, errors, errorRates.prep)

def cz(bit1, bit2, errors, errorRates): 
  cz0(bit1, bit2, errors)
  anisotropic_cz(bit1, bit2, errors, errorRates.cnot)
  errors1(bit1, errors, errorRates.prep)
  errors1(bit2, errors, errorRates.prep)

def anisotropic_cx(bit1, bit2, errors, prob):
    if random.random() < prob:
        # Apply X to bit1 (target), Z to bit2 (control)
        
        errors.z ^= 1 << bit1
        errors.x ^= 1 << bit2
        # errors2(bit1, bit2, errors, prob)

def anisotropic_cz(bit1, bit2, errors, prob):
    if random.random() < prob:
        # Apply Z to both qubits
...
def anisotropic_cy(bit1, bit2, errors, prob):
    if random.random() < prob:
        # Apply Z to both qubits
        errors.x ^= 1 << bit1
        errors.z ^= 1 << bit1
